...
int main() {
    ...
    std::vector<int> hidden_layers_sizes = {2};  // One hidden layer with 3 neurons
    Matrix output_layer(1, 1); // Output layer with 10 neuron (numbers 1-10) for classification
    std::vector <std::string> activation_functions = {"tanH", "sigmoid"}; //activation and output functions, should match be of dim: (1 + number of hidden layers)
    ...
    // Get the data
    std::vector <std::vector<Matrix>> data = get_data_xor();
    ...
    for (int i = 0; i < x_labels_train.size(); i++){
    std::vector<std::vector<Matrix>> predictions = nn.feed_forward_batch(x_labels_train[i]);
    double predicted = predictions[0].back()[0][0];
    double actual = y_labels_train[i][0][0];
