...
void update(std::vector <double>& epochs_n, std::vector <double>& loss_n, std::vector <double>& accuracy_n, double current_accuracy, double current_loss, int width, int height, int epochs, TDT4102::AnimationWindow& window) {
    for (int j = 0; j < epochs_n.size(); ++j) {
        window.draw_circle(TDT4102::Point(50 + (width - 100) / epochs * epochs_n[j], height - 50 - (height - 100) * loss_n[j]*0.1), 2, TDT4102::Color::navy);
        window.draw_circle(TDT4102::Point(50 + (width - 100) / epochs * epochs_n[j], height - 50 - (height - 100) * accuracy_n[j]*0.01), 2, TDT4102::Color::dark_orange);
